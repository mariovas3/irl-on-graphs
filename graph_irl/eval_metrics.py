...
def train_eval_new_policy(
    ...
    (
        ...
        rewards,
        _,
    ) = new_policy.buffer.get_single_ep_rewards_and_weights(
        new_policy.env,
        new_policy,
...
def eval_irl_policy(irl_policy, env, do_graphopt=False):
    ...
    (
        ...
        rewards,
        _,
    ) = irl_policy.buffer.get_single_ep_rewards_and_weights(
        irl_policy.env,
        irl_policy,
...
def get_sum_euc_dist(tgdata, idxs: torch.Tensor = None):
    ...
    print(
        f"num edges x num features for euc dist:",
        tgdata.x[e[0].view(-1, 1), idxs].shape,
    )
    ans = (
        torch.norm(
            tgdata.x[e[0].view(-1, 1), idxs]
            - tgdata.x[e[1].view(-1, 1), idxs],
            p=2,
            dim=-1,
        )
        .sum()
...
def save_graph_stats_k_runs_GO1(
    ...
    adj_list = edge_index_to_adj_list(
        target_graph.edge_index[:, ::2], len(target_graph.x)
    )
    ...
    names_of_stats = ["triangles", "degrees", "clustcoefs"]
    ...
    if euc_dist_idxs is not None:
        if euc_dist_idxs is not None:
            ans = get_sum_euc_dist(target_graph, euc_dist_idxs)
            save_graph_stats(
                target_graph_dir,
                ["eucdist"],
                [ans],
    ...
    for k in range(run_k_times):
        ...
        out_graph, rewards = eval_irl_policy(
            irl_policy, new_policy.env, do_graphopt=do_graphopt
        )
...
def get_means_and_stds(metrics):
    ...
    if ms.ndim < 2:
        ms = ms.reshape(1, -1)
    return np.vstack((ms.mean(0, keepdims=True), ms.std(0, keepdims=True)))
...
def get_stats_of_metrics_and_metrics_in_dict(read_from, verbose=False):
    ...
    summary_stats_over_runs = {
        n: f(s) for (n, s) in summary_stats_over_runs.items()
    }
    for k, v in sorted(
        summary_stats_over_runs.items(), key=lambda x: x[0]
    ):
        print(k, v, sep="\n", end="\n\n")
...
def plot_dists(file_name, groups, suptitle):
    ...
    for i, (metric_name, metric) in enumerate(
        ...
        xlow, xhigh = min(metric) - 0.01, max(metric) + 0.01
        ...
        try:
            ...
            ax.set_xticks(
                np.arange(xlow, xhigh, (xhigh - xlow) / 5).round(1)
            )
            ax.set_xticklabels(ax.get_xticks(), rotation=90)
        except np.linalg.LinAlgError:
            print(
                f"problem with covariance in {metric_name}"
...
def save_analysis_graph_stats(
    test_output_path, file_name, suptitle=None, verbose=False
):
...
def run_on_train_nodes_k_times(
    irl_agent, names_of_stats, k, do_graphopt=False
):
    p = irl_agent.save_to
    ...
    for i in range(k):
        out_graph, rewards = eval_irl_policy(
            irl_agent, irl_agent.env, do_graphopt=do_graphopt
        )
        # get graph stats;
...
def get_mrr_and_avg(
    ...
    for j in range(num_runs):
        ...
        print(
            f"from MRR EVAL, irl-policy did {steps_done} steps and code is {code}"
        )
...
def _get_single_ep_expert_return(
    ...
    return _get_return_and_rewards_on_path(edge_index, nodes, idxs, agent)
...
def _get_return_and_rewards_on_path(
    ...
    while pointer * graphs_per_batch * 2 < edge_index.shape[-1]:
        batch_list = []
        action_idxs = []
        for i in range(
        ...
        if agent.buffer.transform_ is not None:
            ...
            if (
                agent.buffer.transform_.get_graph_level_feats_fn
                is not None
            ):
                extra_graph_level_feats = (
                    agent.buffer.transform_.get_graph_level_feats_fn(batch)
                )
