...
\begin{align}\label{eq:dropPrevRews}
  ...
  \left(
    \sum_{k=1}^{t-1}\gamma^{k-1}r(s_k, a_k)
  \right)\notag\\
  &=\sum_{\tau_{1:t-1}}p_\theta(\tau_{:t-1})\left(
    \sum_{k=1}^{t-1}\gamma^{k-1}r(s_k, a_k)
  \right)\sum_{a_t\in\mathcal{A}}\pi_\theta(a_t|s_t)\nabla \log \pi(a_t|s_t)\sum_{\tau_{t:}}p_\theta(\tau_{t:})\notag\\
  &=\sum_{\tau_{1:t-1}}p_\theta(\tau_{:t-1})\left(
    \sum_{k=1}^{t-1}\gamma^{k-1}r(s_k, a_k)
  \right)\sum_{a_t\in\mathcal{A}}\pi_\theta(a_t|s_t)\nabla \log \pi(a_t|s_t)\notag\\
  &=\sum_{\tau_{1:t-1}}p_\theta(\tau_{:t-1})\left(
...
Intuitively, we prioritise optimising in the direction of actions that 
led to greater returns.
