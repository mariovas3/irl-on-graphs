...
\begin{abstract}
  ...
  After learning a reward function we could use it as an 
  objective on a different graph and perform style-transfer.  
  ...
  properties of rewards given state and actions.
  
  In our experiments, we test two ways to achieve style-transfer 
  on graphs. First, we try  
  directly deploying the 
  jointly learned policy from IRL based on the source graph, 
  on the target graph, and second, 
  we train 
...
\citep{Darvariu,DeepMindGraphGen,GCNPolicyGraphGen,GraphRNN,GraphOpt}.
Some methods 
...
\cite{Darvariu,GraphOpt,GCNPolicyGraphGen}. This work 
is concerned with the latter approach to graph generation. 

In their application 
for generating edges in urban-networks, 
...
Efficiency implies that the graph is ``well-connected'' and 
...
On the other hand, in their application on 
...
Finally, it is worth emphasising the effort that is put into 
...
design a suitable reward signal for very complex problems 
...
the reward function, by exploiting the conditional 
