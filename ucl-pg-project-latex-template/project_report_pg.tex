...
the given trajectory. In this way, ``more optimal'' demonstrations 
will be exponentialy more desirable than others. A simple illustration 
of the soft-optimality idea is portrayed in Figure \ref{fig:softopt}. 
...
\begin{figure}[H]
  ...
  $1$ with probability proportional to $\exp(r(s_t, a_t))$.}
\end{figure}
...
as if it is uniform. This is shown in equation 
\ref{eq:priorMsgs}.
\begin{align}\label{eq:priorMsgs}
  V(s_t)&=\log \int_{\mathcal{A}}\exp (Q(s_t, a_t) + \log p(a_t|s_t))da_t\notag\\
  V(s_t)&=\log \int_{\mathcal{A}}\exp (\hat{Q}(s_t, a_t))da_t.
\end{align}

In which case, the policy equation becomes just the ratio 
of the two messages as shown in Equation \ref{eq:policyMessageNoPrior}.

\begin{equation}\label{eq:policyMessageNoPrior}
