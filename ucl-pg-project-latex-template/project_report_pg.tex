...
\begin{abstract}
  ...
  construction mechanism based on a single graph example. More 
  specifically, we focus on methods of Inverse Reinforcement Learning 
  (IRL) whose goal is to learn a reward function, which when optimised 
  should lead to the construction of graphs similar to the source graph.
  
  We also propose improvements over current methods with respect 
  to decreasing the variance during training of the IRL procedure, 
  which are inspired by the literature on per-decision importance 
  samples.
  
  In our experiments, we test for the efficacy of 
  transfering the topological properties of the source graph 
  to an unseen target graph. We compare two ways to do this. 
  First, we try  
  directly deploying the 
  jointly learned policy from IRL on the target graph, and second, 
  by training 
  a new policy on the target graph using the learned reward 
  function from IRL. In all experiments, we use GraphOpt as 
  our baseline. 
\end{abstract}
\tableofcontents
\setcounter{page}{1}
...
\begin{equation}\label{eq:continualObjective}
  \arg \max_{\pi}\; {E}_{s, a\sim \mu^{\pi}(s, a)}[r(s_t, a_t)],
\end{equation}
...
\begin{align}\label{eq:valFuncDef}
  ...
  &=\mathbb{E}_{s_1\sim p(s_1)}\left[
      \mathbb{E}_{a_1\sim \pi(\cdot|s_1)}\left[
          r(s_1, a_1) + 
          \mathbb{E}_{\tau\sim p^{\pi}(\tau|s_1,a_1)}\left[\sum_{t=2}^{len(\tau)}r(s_t, a_t)\gamma^{t-1}|s_1,a_1\right]
        \right]
    \right]\notag\\
  &=:\mathbb{E}_{s_1\sim p(s_1)}\left[
      \mathbb{E}_{a_1\sim \pi(\cdot|s_1)}\left[Q^{\pi}(s_1,a_1)|s_1,a_1\right]
      \right]\notag\\
      &=:\mathbb{E}_{s_1\sim p(s_1)}\left[
        V^{\pi}(s_1)|s_1
      \right],
...
a reward function given expert trajectories/demonstration 
on performing a given task. Estimating the reward function can be 
...
optimises the 
estimated reward function. Moreover, in some settings, the true 
reward function may be very complex, thereby, 
making it difficult to hand-engineer a good objective for the 
forward RL problem.

There are broadly two popular approaches to IRL. The first one 
\citep{NgIRL}, is based on a heuristic search for reward 
functions such 
that the observed behaviour looks better (through the ``eyes'' of 
the reward) than any other behaviour by the largest 
possible margin. 
This formulation resembles a lot the problem setting for 
support vector machines (SVMs) \citep{SVMs}, 
where the task is to find the 
optimal separating hyperplane.

The authors of \cite{NgIRL} first show that the problem of IRL is 
fundamentally ill-posed. This is because there are many possible 
rewards that make a given policy look optimal. First, they use the 
fact that there always exists at least one optimal policy \citep{Sutton1998} 
such that $V^{\pi}$ is maximised. This implies that if $\pi^*$ is 
an optimal policy and $\pi$ is any other policy, 
$V^{\pi^*}(s)\ge V^{\pi}(s)\;\forall s\in\mathcal{S}$ and a 
similar inequality holds
for the Q-function. Then, with no loss of generality, the 
authors use a state-reward function and rewrite the bellman 
expectation equations in \ref{eq:bellmanv} and \ref{eq:bellmanq} 
to:
\begin{align}
  V^{\pi}(s)&=r(s) + \gamma\sum_{s'\in\mathcal{S}}p()
\end{align}
